...
const Admin_view_med = () => {
const [medData, setMedData] = useState([]);

  useEffect(()=>{
    const fetchData = async()=>{
      try{
      const res = await fetch(`http://localhost:5000/admin-view-med`)
    ...
    <div className='admin-doc-add-container'>
      ...
      {medData.length === 0 ? (
        ...
        <table style={{ width: '70%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {medData.map((med) => (
              <tr key={med.med_dept_id }>
                <td style={{ border: '1px solid #ddd', padding: '8px' , color:'#ddd'}}>{med.med_dept_id}</td>
                <td style={{ border: '1px solid #ddd', padding: '8px', color:'#ddd' }}>{med.med_name}</td>
              </tr>
            ))}
          </tbody>
        </table>
