...
const Ready_patients = () => {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        const response = await fetch('http://localhost:5000/ready-patients'); // your backend API
        if (!response.ok) {
  ...
  return (
    ...
    <div className='all-patients' style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1>Patients Ready To Check-Up</h1>
      {patients.length === 0 ? (
        ...
        <table style={{ width: '70%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f2f2f2' }}>
              ...
              <th style={{ border: '1px solid #ddd', padding: '8px' }}>Doc ID</th>
          ...
          <tbody>
            {patients.map((patient) => (
              <tr key={patient.patient_id}>
                ...
                <td style={{ border: '1px solid #ddd', padding: '8px' }}>{patient.patient_name}</td>
                <td style={{ border: '1px solid #ddd', padding: '8px' }}>{patient.address}</td>
                <td style={{ border: '1px solid #ddd', padding: '8px' }}>{patient.doc_assigned}</td>
