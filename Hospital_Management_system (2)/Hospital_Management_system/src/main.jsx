...
function updateLoadingState(isLoading) {
  window.dispatchEvent(new CustomEvent('fetch-loading', { detail: { loading: isLoading } }));
}
...
window.fetch = async function (resource, init) {
  ...
  const isBackend = resourceUrl && (
    resourceUrl.startsWith('http://localhost:5000') || 
    resourceUrl.startsWith('/') || 
    resourceUrl.startsWith(API_URL)
  );
